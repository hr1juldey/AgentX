...
- Server-driven UI state management with `ui_message_reducer`
...
| FR-FRONTEND-011 | Frontend SHALL render server-driven UI via `push_ui_message()` | Must |
| FR-FRONTEND-012 | Frontend SHALL receive UI events via `onCustomEvent` callback | Must |
| FR-FRONTEND-013 | Components SHALL use Shadow DOM for style isolation | Must |
...
| NFR-FRONTEND-005 | UI state synchronized across all LangGraph nodes | Must |
...
const { state } = useStream({
  onCustomEvent: (event) => {
    if (event.type === "ui_update") {
      ui_message_reducer(updateUIState(event.data))
    }
  }
})
```
...
| BR-FRONTEND-005 | Server-driven UI prevents state desync | Code review |
| BR-FRONTEND-006 | Designer agent accesses state.ui | Code review |
...
- [ ] Server-driven UI renders via push_ui_message()
- [ ] UI state synchronized across all nodes
- [ ] Designer agent can access state.ui

## 1.8 Scenarios

### Scenario: Component rendering (from C007 exploration)
...
### Scenario: State awareness (from C007 exploration)
...
### Scenario: Design tokens (from C008 exploration)

...
class PasswordEntryUpdate(BaseModel):
    ...
    title: Optional[str] = Field(
        None, description="Updated entry title", min_length=1, max_length=200
    )
    username: Optional[str] = Field(
        None, description="Updated username", max_length=100
    )
    ...
    url: Optional[str] = Field(None, description="Updated service URL", max_length=500)
...
class PasswordEntryResponse(PasswordEntryBase):
    ...
    id: int = Field(..., description="Unique entry identifier", examples=[1, 42])
    user_id: int = Field(..., description="Owner user ID", examples=[1])
    created_at: datetime = Field(
        ..., description="When the entry was created", examples=["2024-01-15T10:00:00Z"]
    )

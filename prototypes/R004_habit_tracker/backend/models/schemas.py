...
class HabitResponse(BaseModel):
    ...
    description: str | None = Field(
        ...,
        description="Habit description or null"
    )
    frequency: HabitFrequency = Field(
        ...,
        description="Habit frequency",
        examples=[HabitFrequency.DAILY]
    )
    target_count: int = Field(
        ...,
    ...
    total_completions: int = Field(
        ...,
        description="All-time total completions",
        examples=[100, 365],
        ge=0
    )
...
class HabitCompletionResponse(BaseModel):
    ...
    habit_id: int = Field(
        ...,
        description="ID of the habit that was completed",
        examples=[1]
    )
    ...
    notes: str | None = Field(
        ...,
        description="Completion notes or null"
    )
...
class HabitDetailResponse(HabitResponse):
    ...
    completions: list[HabitCompletionResponse] = Field(
        ...
        description="Recent completion history (chronological)"
    )
...
class TimeSeriesData(BaseModel):
    ...
    count: int = Field(
        ...,
        description="Number of completions on this date",
        examples=[3],
        ge=0
    )
...
class HabitListResponse(BaseModel):
    ...
    habits: list[HabitResponse] = Field(
        default_factory=list,
        description="List of habits (may be empty)"
    )
    total: int = Field(
        ...,
        description="Total count of habits",
        examples=[10]
    )
